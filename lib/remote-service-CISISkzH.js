...
*/
var SolutionCouncilService = class extends TypertRemoteService {
	...
	async orchestrate(agent, initial, parentSignal) {
		...
		try {
			...
			const review = await this.runReportAgent(agent, "交叉评审", `你是方案团的串行交叉评审者。你必须保持独立上下文，只审查下面几份同职责调查报告，指出共识、分歧、证据强弱与遗漏。必要时可以用只读工具回到代码库核验。不要复述思维过程，只输出结构化报告。\n\n任务：${run.task}\n调查报告：${compact(reports)}`, controller.signal);
			run = await this.updateRun(agent, run.runId, (current) => ({
				...current,
				review
			}));
			...
			const verification = await this.runReportAgent(agent, "证据核验", `你是方案团的串行证据核验者。根据任务、调查报告和交叉评审，逐项检查关键结论是否有代码证据，标出不能确认的地方和风险。你可以使用只读工具核验代码。只输出结构化报告。\n\n任务：${run.task}\n调查报告：${compact(reports)}\n交叉评审：${compact(review)}`, controller.signal);
			run = await this.updateRun(agent, run.runId, (current) => ({
				...current,
				verification
			}));
			...
			const final = await this.runFinalAgent(agent, `你是主 AI 的串行最终裁判。综合同职责调查、交叉评审和证据核验，给出一个可执行的最终方案。必须明确：建议做什么、为什么、未解决的风险是什么。不要把多数票当成真相；证据质量优先。只输出结构化最终决定。\n\n任务：${run.task}\n调查报告：${compact(reports)}\n交叉评审：${compact(review)}\n证据核验：${compact(verification)}`, controller.signal);
	...
	async runReportAgent(agent, label, prompt, signal) {
		try {
			return reportFromChild((await this.startChild(agent, label, prompt, signal, REPORT_OUTPUT_SCHEMA)).result, `${label} 未返回有效报告。`);
		} catch (cause) {
			return {
	...
	async startChild(parent, label, prompt, signal, outputSchema) {
...
var SolutionCouncilRemoteService = class extends SolutionCouncilService {
	constructor(...args) {
		super(...args);
	...
	remoteGet(agent, callId) {
		return this.getByCall(agent, callId);
	}
	remoteCancel(agent, callId) {
...
//#endregion
export { name as a, inject as i, Config as n, SolutionCouncilService as r, SolutionCouncilRemoteService as t };
